...
## Contexto
El esquema de la base de datos PostgreSQL está estrictamente definido en el archivo `backup5.sql`. Este archivo es la ÚNICA fuente de verdad para nombres de tablas, columnas, tipos de datos y relaciones.
...
1.  **LECTURA OBLIGATORIA:** Debes leer y analizar el contenido de `backup5.sql` para validar el esquema actual.
2.  **VALIDACIÓN DE EXISTENCIA:** No asumas la existencia de tablas o columnas. Si no está en `backup5.sql`, no existe.
...
## Manejo de Errores
Si el código solicitado implica una tabla o columna que no encuentras en `backup5.sql`:
- **DETENTE.**
