...
async function analizarRiesgoSolicitud(solicitudId) {
...
async function obtenerSolicitudesPendientes(req, res) {
  try {
    ...
    for (const solicitud of solicitudes) {
      const analisis = await analizarRiesgoSolicitud(solicitud.solicitud_id);
...
async function analizarRiesgoCredito(req, res) {
  try {
    ...
    const pedidosQuery = `SELECT montototal, fechapedido FROM pedidos WHERE clienteid = $1 AND pagado = true ORDER BY fechapedido DESC`;
    const { rows: pedidos } = await db.query(pedidosQuery, [solicitud.cliente_id]);
    ...
    const totalCompras = pedidos.reduce((sum, p) => sum + parseFloat(p.montototal), 0);
    const promedioCompras = antiguedadMeses > 0 ? totalCompras / antiguedadMeses : 0;
    const evaluacion = await analizarRiesgoSolicitud(solicitudId);
...
async function rechazarSolicitud(req, res) {
  try {
    ...
    const result = await db.query("UPDATE solicitudes_credito SET estado = 'RECHAZADO', comentarios_admin = $1 WHERE solicitud_id = $2 AND estado = 'PENDIENTE'", [motivo_rechazo, solicitud_id]);
