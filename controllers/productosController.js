...
const obtenerProductos = async (req, res) => {
  try {
    ...
    if (varianteIds.length > 0 && req.user && req.user.id && req.tenant) {
      try {
        // 🚀 OPTIMIZACIÓN: Usar getBulkStock para TODOS (clientes y admins)
        // Se eliminó el loop N+1 que hacia calculateAllocationStatus por cada variante
        stockMapByRole = await SmartStockService.getBulkStock({
          varianteIds,
          userId: req.user.userId || req.user.id,
          userRole: req.user.roles || [req.user.rol || 'cliente'],
          tenantId: req.tenant.tenant_id,
...
const obtenerDimensiones = async (req, res) => {
  try {
    ...
    const dimensiones = result.rows.map((row) => row.dimensiones);
