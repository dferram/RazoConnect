/**
 * MÓDULO DE GENERACIÓN DE REMISIONES PARA INVENTARIOS
 * Versión sin precios para el rol de inventarios
...
async function abrirModalGenerarRemisionInventarios(pedidoId) {
    try {
        ...
        mostrarModalRemisionInventarios();
...
function mostrarModalRemisionInventarios() {
    // Verificar si el usuario es del rol inventarios
    ...
    Swal.fire({
        ...
        didOpen: () => {
            configurarEventosModalInventarios();
            if (!isInventarios) {
                calcularTotalRemisionInventarios();
            }
        },
        preConfirm: () => {
            return validarYObtenerDatosInventarios();
        }
    }).then((result) => {
        if (result.isConfirmed && result.value) {
            generarRemisionAPIInventarios(result.value);
        }
    });
}

function configurarEventosModalInventarios() {
...
function calcularTotalRemisionInventarios() {
    let total = 0;
...
function validarYObtenerDatosInventarios() {
    const checkboxes = document.querySelectorAll('.item-checkbox:checked');
...
async function generarRemisionAPIInventarios(datos) {
    try {
        Swal.fire({
...
// Exponer función global para que pueda ser llamada desde admin-pedidos.html
window.abrirModalGenerarRemision = abrirModalGenerarRemisionInventarios;
