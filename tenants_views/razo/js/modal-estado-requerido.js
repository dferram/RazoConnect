...
async function inicializarModalEstadoRequerido() {
  try {
    ...
    // Mostrar modal forzoso si no tiene estado
    await mostrarModalEstadoRequerido();
  } catch (error) {
    console.error('❌ Error en inicializarModalEstadoRequerido:', error);
  }
}

/**
 * Crea y muestra el modal de selección de estado
 */
async function mostrarModalEstadoRequerido() {
  try {
    ...
    console.log('✅ Modal DOM creado y agregado al body');
    ...
    console.error('❌ Error en mostrarModalEstadoRequerido:', error);
  }
}
